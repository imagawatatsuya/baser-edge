...
function validateGalleryV2Props(props) {
  ...
  items.forEach((item, index2) => {
    ...
    validateAccessibleImageProps(item).forEach((message) => {
      errors.push(`items[${index2}]: ${message}`);
    });
  });
  return errors;
}
...
function buildAbsoluteCanonicalUrl(origin, path) {
  const base = origin.replace(/\/+$/, "");
  const normalized = normalizePublicPath(path);
  return normalized === "/" ? `${base}/` : `${base}${normalized}`;
}
function trimMetaDescription(text, maxLength = 300) {
  const collapsed = text.replace(/\s+/g, " ").trim();
  if (collapsed.length <= maxLength)
    return collapsed;
  return `${collapsed.slice(0, maxLength - 1).trimEnd()}…`;
}
function descriptionFromDocument(document) {
  const parts = [];
  for (const block of flattenBlocks(document)) {
    ...
    if (block.type === "imageText" && typeof block.props.text === "string")
      parts.push(block.props.text);
  }
...
function renderSeoHeadHtml(pageTitlePlain, seo) {
  const description = trimMetaDescription(seo.description || pageTitlePlain);
  const robots = seo.preview ? "noindex, nofollow, noarchive" : "index, follow";
  ...
  if (seo.datePublished)
    jsonLd.datePublished = seo.datePublished;
  if (seo.dateModified)
    jsonLd.dateModified = seo.dateModified;
  lines.push(`<script type="application/ld+json">${serializeJsonLd(jsonLd)}<\/script>`);
  return lines.join("\n");
}
function extractTitleForSeo(title, siteName) {
  if (!title && siteName)
    return siteName;
...
function renderDocument(document, resolver = defaultResolver, options = {}) {
  ...
  const imageState = { firstContentImageSeen: false };
  return Object.values(document.root.slots).flat().map((block) => renderBlock(block, resolver, now, options.preview ?? false, imageState)).join("\n");
}
function renderPage(document, resolver = defaultResolver, options = {}) {
  ...
  return renderShell({
    ...
    bodyHtml: body,
    theme,
    ...options.siteName !== void 0 ? { siteName } : {},
    lang,
    ...options.headHtml !== void 0 ? { headHtml: options.headHtml } : {},
    ...options.seo !== void 0 ? { seo: options.seo } : {},
    bodyAttributes
  });
}
...
function renderBlock(block, resolver, now, preview, imageState) {
  ...
  switch (block.type) {
    ...
    case "image": {
      ...
      if (block.componentVersion >= 2) {
        ...
        const captionHtml = caption ? `<figcaption>${escapeHtml$2(caption)}</figcaption>` : "";
    ...
    case "imageText": {
      ...
      if (block.componentVersion >= 2) {
        const decorative = block.props.decorative === true;
        const alt = decorative ? "" : asString(block.props.alt);
        return `<section data-block-id="${id}" class="bc-image-text"><img src="${escapeAttribute(src)}" alt="${escapeAttribute(alt)}"${loading}><p>${escapeHtml$2(asString(block.props.text))}</p></section>`;
      }
      return `<section data-block-id="${id}" class="bc-image-text"><img src="${escapeAttribute(src)}" alt="${escapeAttribute(asString(block.props.alt))}"${loading}><p>${escapeHtml$2(asString(block.props.text))}</p></section>`;
    }
    case "gallery": {
      if (block.componentVersion >= 2) {
    ...
    case "table": {
      if (block.componentVersion >= 2) {
        const caption = escapeHtml$2(asString(block.props.caption));
        const headers = Array.isArray(block.props.columnHeaders) ? block.props.columnHeaders.map((value) => escapeHtml$2(String(value))) : [];
        const rows2 = Array.isArray(block.props.rows) ? block.props.rows : [];
        const rowHeaderColumn = block.props.rowHeaderColumn === true;
        const thead = headers.length ? `<thead><tr>${headers.map((header) => `<th scope="col">${header}</th>`).join("")}</tr></thead>` : "";
        const tbody = rows2.map((row) => {
          const cells = Array.isArray(row) ? row.map((cell) => escapeHtml$2(String(cell))) : [];
...
function imageLoadingAttr(block, imageState, preview) {
...
function buildPublicPageSeo(input) {
  const description = trimMetaDescription(descriptionFromDocument(input.document) || input.title || input.siteName);
  const canonicalUrl = buildAbsoluteCanonicalUrl(input.origin, input.path);
  const dateIso = input.revision ? new Date(input.revision.createdAt).toISOString() : void 0;
  return {
    description,
    canonicalUrl,
    locale: input.locale,
...
const SITEMAP_INDEXABLE_TYPES = /* @__PURE__ */ new Set(["page", "blog", "article", "custom-content", "mail-form"]);
...
async function renderSitemapXml(cms, siteId, origin) {
  const entries = await cms.store.listContentTree(siteId);
  const urls = [];
  for (const entry of entries) {
    const snapshot = entry.snapshot;
    const item = snapshot.item;
    if (item.state !== "active" || !snapshot.publishedRevision)
      continue;
    if (snapshot.route.routeType !== "canonical" || !snapshot.route.isCanonical || !snapshot.route.active)
      continue;
    if (!SITEMAP_INDEXABLE_TYPES.has(item.contentTypeKey))
      continue;
    ...
    async fetch(request, env, context) {
      try {
        ...
        if (url.pathname === "/sitemap.xml") {
          const xml2 = await renderSitemapXml(cms, siteId, url.origin);
          return new Response(request.method === "HEAD" ? null : xml2, {
        ...
        if (customDetailMatch) {
          ...
          if (root?.kind === "content" && root.snapshot.item.contentTypeKey === "custom-content" && root.snapshot.publishedRevision) {
            ...
            if (definition2) {
              ...
              if (entry)
                return renderCustomEntryDetail(request, root.snapshot, definition2, entry, await customContent.getTableSchema(definition2.tableId), activeTheme, siteName, siteLocale);
            }
          }
        }
        ...
        if (snapshot.item.contentTypeKey === "custom-content") {
          ...
          if (!definition2)
            return new Response("Not Found", { status: 404 });
          return renderCustomContentList(request, snapshot, definition2, customContent, url, activeTheme, siteName, siteLocale);
        }
...
async function renderBlogResponse(request, snapshot, blog, collectionId, assets, url, theme, siteName, siteLocale, termIds = [], suffixTitle = "") {
...
async function renderCustomContentList(request, snapshot, definition2, service, url, theme, siteName, siteLocale) {
  if (!snapshot.publishedRevision)
    return new Response("Not Found", { status: 404 });
...
function renderCustomEntryDetail(request, snapshot, definition2, entry, schema, theme, siteName, siteLocale) {
  const revision = entry.publishedRevision;
  const displayKey = schema.table.displayFieldKey ?? schema.fields[0]?.definition.key;
  const title = displayKey ? displayValue(revision.values[displayKey]) : entry.entry.id;
...
function renderMailForm(request, snapshot, form, schema, theme, siteName, siteLocale, turnstileSiteKey) {
