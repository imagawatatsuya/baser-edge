...
| `/v1/content/:id/reorder` | POST | Y | Y | Y | OK | Success + stale `expectedTreeVersion` + invalid sibling anchor in `api-worker.test.mjs`; Article cross-Blog move stays 409 in `blog.test.mjs` |
