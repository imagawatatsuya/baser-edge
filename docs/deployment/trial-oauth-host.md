...
- **既定（`BASER_TRIAL_PROVISION_STRATEGY=release`）:** メンテナが配布する **固定ビルド成果物** を trial-host から取得し、利用者アカウントへ **D1 + Worker REST デプロイ**（**利用者の GitHub アカウント不要**）。`deploy:trial-host` 前に `npm run pack:trial-release` が実行されます。
- 開設処理は `ctx.waitUntil()` や単一 Queue invocation では実行しません。D1、マイグレーション40 SQLごと、Assets、Worker、Secrets、bootstrapを **別々の Queue メッセージ**に分割し、Workers Free の外部 subrequest 上限（50/invocation）未満に保ちます。
